...
class Slider {
    ...
    draw(x, y) {
        ...
        rect(
            ...
            this.y,
            this.width *
                ((this.value - this.beginVal) / (this.endVal - this.beginVal)),
            this.height,
            this.radius
        )
        ...
        if (
            Math.sqrt(
                Math.pow(mouseX - this.circleX, 2) +
                    Math.pow(mouseY - this.circleY, 2)
            ) < this.circleRadius
        )
            stroke(SLIDER_OVER_CIRCLE_BORDER_COLOR)
    ...
    mouseDragged() {
        if (this.locked) {
            ...
            this.value =
                Math.floor(
                    ((this.circleX - this.x) / this.width) *
                        (this.endVal - this.beginVal)
                ) + this.beginVal
